...
# model code adapted from https://github.com/rodekruis/GlobalTropicalCycloneModel/blob/main/analysis/04_codes_of_article/07_grid_To_mun_2SG-Global%2B.ipynb
from model_function import Load_Model
from imblearn.under_sampling import RandomUnderSampler
from xgboost.sklearn import XGBRegressor
from xgboost import XGBClassifier

def predict2SGGlobal(X_test, X_train = Load_Model("X_train"), y_train = Load_Model("y_train")):
    ...
    under = RandomUnderSampler(sampling_strategy=0.1, random_state=56) #added random state
    ...
    reduced_df = pd.DataFrame(X_train.copy(), columns=features)
    ...
    reduced_test_df = pd.DataFrame(X_test.copy(), columns=features)
    reduced_test_df["predicted_value"] = xgb_model.predict(X_test)

    # damaged prediction (df1) / not damaged prediction (df2)
    ...
    y1_pred = xgbR.predict(X1) # reduced xgb model for pred = 1
    ...
    y0_pred = xgb.predict(X0) # full xgb model for pred = 0
    ...
    return join_test_dfs["predicted_percent_damage"]
...
def eval_bins(ypred, ytrue, ybins):
    ...
    for v in bins:
        ...
        #average error
        avge = np.mean( y_v['y_pred'] - y_v['y_true'])
        mae = mean_absolute_error(y_v['y_true'], y_v['y_pred'])
        eval.append([v, mse, rmse, mae, avge])

        print("Bin: {:<15} RMSE: {:.3f}, Avg. Error: {:.3f}".format(str(v), rmse, avge))

    print("Total RSME", np.sqrt(mean_squared_error(y_df['y_true'], y_df['y_pred'])))
    print("Total Avg. Error", np.mean(y_df['y_pred'] - y_df['y_true']))
    #print("Total MAE", mean_absolute_error(y_df['y_true'], y_df['y_pred']))
